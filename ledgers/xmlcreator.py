...
def cmxml(data, request,vendor_code, group, addresses):
# Party_master
    ...
    m3.text= str(data.state.name)
    root.append (m3)

    m4 = Et.Element("COUNTRYNAME")
    m4.text = str(data.country.name)
    root.append(m4)

    m5 = Et.Element("PINCODE")
    m5.text = str(data.pin_code)
    root.append(m5)
    ...
    for i in addresses:
        if i:
            ...
            f1.text = str(i.addressline1)

            f2 =Et.SubElement(e8, "ADDRESS")
            f2.text = str(i.addressline2)

            f3 =Et.SubElement(e8, "ADDRESS")
            f3.text = str(i.addressline3)

            e9 = Et.SubElement(m23, "EMAIL")
            e9.text = str(i.email_id)
            e10 = Et.SubElement(m23, "PINCODE")
            e10.text = str(i.pin_code)
            e11= Et.SubElement(m23, "PHONENUMBER")
            e11.text = str(i.phone_no)
            e12= Et.SubElement(m23, "MOBILENUMBER")
            e12.text = str(i.mobile_no)
            e13 = Et.SubElement(m23, "FAXNUMBER")
            e13.text = str(i.fax_no)
            e14 = Et.SubElement(m23, "INCOMETAXNUMBER")
            e14.text = str(i.pan_no)
            e15 = Et.SubElement(m23, "COUNTRYNAME")
            e15.text = str(i.country)
            e16 = Et.SubElement(m23, "ADDRESSNAME")
            e16.text = str(i.address_type)
            e17 = Et.SubElement(m23, "PARTYGSTIN")
            e17.text = str(i.gstin)
            e18 = Et.SubElement(m23, "CONTACTPERSON")
            e18.text = str(i.contact_person)
            e19 = Et.SubElement(m23, "STATE")
            e19.text = str(i.state)
            e20 = Et.SubElement(m23, "GSTREGISTRATIONTYPE")
            e20.text = str(i.gst_registration_type)
