...
def brandxml(data):
#Brand
    root = Et.Element("STOCKGROUP",{"NAME":data.name,"GUID":str('e20c8366-3352-44fd-ab5b-f697e4b53794-000090ba'),"RESERVEDNAME":""})

    j1=Et.Element("PARENT")
    j1.text=""
    root.append(j1)

    j2=Et.Element("ISUPDATINGTARGETID")
    j2.text="No"
    root.append(j2)
...
#category
    root = Et.Element("STOCKCATEGORY",{"NAME":data.name,"RESERVEDNAME":""})
    ...
    j2=Et.Element("OBJECTUPDATEACTION")
    j2.text='Alter'
    root.append(j2)

    j2=Et.Element("ALTERID")
    j2.text='2532'
    root.append(j2)
...
def editcategoryxml(data, old):
#category
    root = Et.Element("STOCKCATEGORY",{"NAME":old,"RESERVEDNAME":""})
...
def godownxml(data):
#Godown
    root = Et.Element("GODOWN",{"NAME":data.name,"RESERVEDNAME":""})

    j1=Et.Element("PARENT")
    if data.parent.name == "Primary":
        j1.text=''
    else:
        j1.text=data.parent.name
    root.append(j1)
...
#produictMaster
	...
	m11.text=data.units_of_measure.symbol
	root.append(m11)

	if data.additional_units:
		m12=Et.Element("ADDITIONALUNITS")
		m12.text=data.additional_units.symbol
		root.append(m12)
	...
	for i in gstdetail:
		...
		b1.text=i.applicable_from.strftime('%Y%m%d')
